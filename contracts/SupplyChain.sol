...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale (uint sku) {
    Item storage j = items[sku];
    require(j.price != 0 && j.state == State.ForSale); _; 
    }
  modifier sold (uint sku) {
    Item storage j = items[sku];
    require(j.state == State.Sold); _;
    }
  modifier shipped (uint sku) {
    Item storage j = items[sku];
    require(j.state == State.Shipped); _;
    }
  modifier received (uint sku) {
    Item storage j = items[sku];
    require(j.state == State.Received); _;
    }
    ...
    checkValue(_sku)
  {
    ...
    j.state = State.Sold;
    j.buyer = msg.sender;
    j.seller.transfer(items[_sku].price);
    emit LogSold (_sku);
  }
